// server.js - FULL Production File (LICENSE KEY PATCHED FOR ALL INSERTS)
...
app.post('/stripe/update-license', async (req, res) => {
  ...
  try {
    ...
    if (data && data.length > 0) {
      ...
      // PATCH: Ensure license_key is present for paid/stripe
      const license_key = licenseData.license_key || `paid_${email.replace(/[^a-zA-Z0-9]/g, '')}_${Date.now()}`;
      const { error: insertError } = await supabase
        .from('licenses')
        .insert([{ ...licenseData, email, license_key, is_active: true }]);
...
app.post('/api/register-free-user', async (req, res) => {
  ...
  try {
    ...
    // PATCH: Generate license_key for every new free registration
    const license_key = `free_${email.replace(/[^a-zA-Z0-9]/g, '')}_${Date.now()}`;
    const { error: insertError } = await supabase
      .from('licenses')
      .insert([
        { email, license_key, license_type: 'free', status: 'active', created_at: new Date().toISOString() }
      ]);
